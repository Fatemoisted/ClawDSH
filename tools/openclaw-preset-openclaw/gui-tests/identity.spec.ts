...
const assemblyRoot = join(repositoryRoot, 'tools/openclaw-preset-openclaw')
...
function runRefresh(home: string): SpawnSyncReturns<string> {
  return spawnSync(linkScript, [], {
    cwd: repositoryRoot,
    encoding: 'utf8',
    env: { ...process.env, CLAWDSH_SKIP_BUILD: '1', DSH_HOME: home },
  })
}
...
describe('ClawDSH installed profile identity', () => {
  ...
  it.each(['channel-feishu', 'channel-telegram', 'channel-discord', 'automation'])(
