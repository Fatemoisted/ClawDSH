...
Delete the `DSH_CI_FAILOVER_LINUX` or `DSH_CI_FAILOVER_WINDOWS` variable (or set it to anything other than `selfhosted`). New canonical-repository runs resolve back to the hosted enterprise pools; mirror runs resolve back to standard hosted capacity. Remove any extra instances that were registered during the incident.
