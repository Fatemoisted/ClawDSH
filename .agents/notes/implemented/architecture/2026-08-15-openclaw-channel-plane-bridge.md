...
The Gateway becomes ready only after authentication and durable route-transition recovery. A transient transport detach rejects socket-owned waits and new calls but lets admitted handlers settle into durable Agent state; progress remains bound to the detached peer. Explicit Provider shutdown aborts and drains handlers owned by active and detached peers before storage closes. OpenClaw registry instances lease one process-shared transport with an immutable configuration identity, and the last lease drains it so a later start receives a fresh connection.
...
The current sidecar is not installable, certified, or enabled at any individual Channel. The production profile contains the complete canonical seam and its invariant companions in the canonical channel-plane composition while the Gateway setting remains disabled; a separately default-disabled legacy compatibility group registers only `ctx.legacyChannels`, and neither path starts a transport by default. Legacy opt-in makes canonical Gateway startup and Settings preflight fail loudly before side effects. Explicit TypeScript source mappings keep all three companion specifiers on the source plane in a clean checkout, while the public bundle pins `@deepseek-ai/dsh-invariants@0.1.0-rc.6`. An owned keyless smoke validates safe Telegram and Feishu configuration against the real stable schema, traverses the locked Gateway and real DSH Agent, and runs in Linux x64 CI; the reviewed Darwin arm64 assembly has also passed it. No fresh credentialed Telegram, Feishu, or Discord sidecar traffic has run. Historical credentialed legacy Telegram and Feishu traffic and keyless Discord evidence cannot certify the locked host, `ctx.channels`, or the sidecar execution path. Stable V1 cannot project safely staged inbound media; the locked host exposes neither a correlated final-answer delivery hook nor aggregate account health; external reviews remain pending; and persistence and resume evidence must prove the known-event degradation above. The legacy packages remain separately available under the non-conflicting `ctx.legacyChannels` Service until the live smokes and remaining replacement conditions pass.

Channel implementation follows the contract-first Harness reuse rule in [ADR-0010](../../../../docs/adr/0010-harness-contract-first.md). [ADR-0011](../../../../docs/adr/0011-deferred-channel-images-and-address-continuity.md) governs image import and address continuity only for the retained legacy compatibility path; its tests and evidence do not extend the canonical sidecar's support state.
