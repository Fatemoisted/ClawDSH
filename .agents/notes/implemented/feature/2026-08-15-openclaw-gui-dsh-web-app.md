# Agent Note: OpenClaw GUI frontend via the dsh web bundle (dsh-web-app + openclaw preset)
...
The `preset-openclaw` profile now composes the web bundle and ships a full agent preset, so `pnpm dsh --profile openclaw` boots the browser GUI with OpenClaw's features. Four changes, all in ClawDSH-owned files:
...
- **default preset** — `profile/cordis.patch.yml` overrides the web bundle's `agent-presets` `default` from `standard` to `openclaw`, so new GUI sessions mount the OpenClaw shape automatically.
...
- **preset installation** — `tools/link-openclaw.sh` gains a third step that copies `preset.yml` + `agent.cordis.yml` + `souls/` into `$DSH_HOME/.agent-presets/openclaw/` (the preset id is the directory name).

## Consequences

- `pnpm dsh --profile openclaw` now serves `http://127.0.0.1:3080`; a GUI conversation surfaces the same soul persona, `clawdsh:memory-recall` section, `memory_search`/`memory_get` tools, and skills catalog as the channel path does, plus the full standard toolset.
- The Feishu daemon behavior is unchanged except that the profile now also starts the webserver; the channel rows and the memory/skills/automation rows are untouched.
...
**Keep the `standard` preset and only override its persona.** Rejected: the web bundle disables the host-plane tool rows, so the `standard` preset is the only thing exposing shell/fs/skills under the GUI; copying it into the openclaw preset with soul-for-persona keeps the OpenClaw identity without forking the tool wiring.

**Select the preset per-session in the UI instead of changing the default.** Rejected as the only path: a per-session pick stays available, but setting `default: openclaw` makes the GUI show the OpenClaw shape with no manual step.
