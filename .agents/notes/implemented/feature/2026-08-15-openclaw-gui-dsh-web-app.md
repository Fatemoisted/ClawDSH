...
The physical `tools/openclaw-preset-openclaw/` assembly composes the web bundle and ships the full `clawdsh` agent preset, so `pnpm dsh --profile clawdsh` boots the browser GUI with ClawDSH's OpenClaw-derived features. Four changes, all in ClawDSH-owned files:
...
- The Web server starts independently of optional external integrations. Feishu, Telegram, Discord, and Automation are disabled in a clean install; enabling their Loader entries preserves their existing behavior and validation.
