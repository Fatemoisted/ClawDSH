...
The profile contains both the canonical `clawdsh-communication-plane` group and the compatibility-only `clawdsh-legacy-channel-plane` group, with both disabled by default; Automation also remains disabled. The legacy group contains channel-core plus Telegram, Discord, and Feishu and requires a master switch plus a per-adapter switch. While its master opt-in is present, canonical Gateway startup and Settings preflight fail loudly before side effects. A clean home can therefore start the Web application without channel, Ark, or automation credentials and without creating external channel or scheduled-run side effects. Enabling a credentialed adapter remains an explicit configuration action; its existing fail-loud credential validation applies when it is mounted. The [test1 rebuild note](../architecture/2026-08-15-test1-channel-plane-rebuild.md) owns the two channel Services and their mutual exclusion.
...
- Clean-home GUI startup is independent of optional channel and automation credentials; both communication planes and Automation remain explicit opt-ins at the Loader layer.
