...
- NO_REPLY: the default prompt asks for it; the observer logs it at info level. Delivery suppression is channel-core's `extractReply` plugin-turn filter — turns whose claimed `user/message` is plugin-sourced never replace the channel reply (also future-proofs schedule notices on channel sessions).
...
- channel-core's `extractReply` now filters plugin-sourced turns — a behavior change to the channel reply extraction path, covered by a channel-core test that queues a plugin-sourced followup inside a driveTurn span.
