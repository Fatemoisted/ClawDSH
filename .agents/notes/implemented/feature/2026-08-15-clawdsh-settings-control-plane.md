...
The Settings page begins with one configuration-and-data ownership guide rather than creating another persistence layer. Non-secret product settings live in `$DSH_HOME/settings.yaml`; DeepSeek and Ark values live in dsh credential sources; deployment composition lives in the installed bundle plus profile/home patches; and platform accounts and policy live in `$DSH_HOME/clawdsh/channel/openclaw/state/openclaw.json` under OpenClaw ownership. Skills Hub's compatible managed directory remains `~/.clawdbot/skills` outside `$DSH_HOME` by default and is moved only through its editable `managedDir` field.

`SETTINGS_MANIFEST` contains exactly the eight namespaces in the table and must classify every leaf in each registered Config schema as editable, installer-managed, or explicitly hidden. The browser cannot infer or add a setting path. `CREDENTIAL_MANIFEST` exposes only `ARK_API_KEY` in the ClawDSH section; DeepSeek model credentials remain on native Models settings, and OpenClaw platform credentials remain outside the RPC.
...
Ark Embeddings uses the fixed `ARK_API_KEY` credential reference and resolves it for each call. It does not accept a literal API key setting. A change to `$DSH_HOME/.credentials.yaml` is visible to the next credential resolution; inherited environment and either `.env` layer are launch snapshots and require restart. The credential RPC allowlist exposes only credentials owned by dsh, reports configured and writable state without a value, and never includes Feishu, Telegram, or another OpenClaw platform credential. OpenClaw exclusively owns those platform secrets, accounts, and state; they do not enter dsh credentials, Settings RPC, browser state, logs, Session files, or Activity storage.

The managed credential document uses owner-only directory and file permissions. Mode `0600` protects it from other operating-system users, not from an Agent tool deliberately reading files as the same UID. The product never supplies its resolved path or value to the model, but that narrower guarantee is not an operating-system sandbox against same-user tools.
...
Protocol and runtime tests pin exact-object parsing, exact eight-namespace manifest coverage, complete schema-leaf classification, the sole Ark credential entry, static allowlists, pollution-path rejection, stale-revision conflicts, reset layering, desired/runtime value comparison, preflight-before-persist ordering, and secret-free credential responses. Capability tests pin that disabled plugins remain describable while their business effects are absent, that Ark resolves only the fixed reference, and that managed-file changes and launch snapshots have different effect timing.

Browser tests pin the configuration-and-data ownership guide, independent drafts, schema controls, the atomic Automation editor, managed Gateway fields, conflict-and-reload behavior, and credential cleanup after both successful and failed requests. The keyless real-profile journey starts without an OpenClaw artifact or external credential, discovers every mounted capability namespace, confirms Gateway is disabled, and confirms that platform credentials are absent from the product control plane. Focused Host tests cover mutation, reset, restart state, stale-write rejection, and the absence of secret values from RPC, rendered DOM, logs, Activity, and profile dumps.
