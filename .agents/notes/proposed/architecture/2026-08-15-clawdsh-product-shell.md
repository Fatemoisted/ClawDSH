...
The physical `tools/openclaw-preset-openclaw/` directory remains temporarily because the repository's current hierarchy gate recognizes that assembly path. Product copy, installed profile and preset ids, default selection, commands, safe clean-install defaults, and legacy handling follow the [ClawDSH identity decision](../../implemented/feature/2026-08-15-clawdsh-identity-and-safe-defaults.md); the directory name is not exposed as a compatibility promise.
...
- The retained `tools/openclaw-preset-openclaw/` path can leak obsolete branding into commands or UI. Identity tests must inspect installed assets and rendered copy while treating the directory name as internal only.
