...
| **Channel gateway (Gateway)** | `src/gateway/` | **none** | **new seam** | `channel-core` | **implemented** (awaited durability, deterministic resume/preset/FIFO, legacy thread-only compatibility, `groupMode`/structured-mention policy, exact-model image-modality check and durable Harness attachment references ✅) |
| Channel: Telegram | `src/telegram/` | `ctx.channels` | Plugin | `channel-telegram` | **implemented; credentialed direct/group text/caption e2e verified** (photo/image-document import and text-only no-download behavior are keyless-tested, not live-tested; forum-topic live coverage remains deferred; [package details](../../packages/openclaw/channel-telegram/README.md)) |
