...
|---|---|---|---|---|
| [`channel-core`](../../packages/openclaw/channel-core/README.md) | Defines `ctx.channels`; routes provider conversations and accepted image references into durable agent sessions | Harness: `agents`, `sessions`, `llm`, `agentDefaultModel`, `agentPresets`, `sessionPersistence`, `timer` | Harness: agent create/resume, preset resolve/mount, exact-model `resolveModelInfo`, LLM text/image content blocks, attachment reference types, session flush, timeout utility | — |
| [`channel-telegram`](../../packages/openclaw/channel-telegram/README.md) | Telegram event, image-materialization, and send/reaction adapter | ClawDSH: `channels`; Harness: `timer` | Harness: optional `credentials`, launch environment, `credentials/updated`, and `ctx.attachments` limits/validate/save | grammY, `@grammyjs/auto-retry`, `@grammyjs/files` |
