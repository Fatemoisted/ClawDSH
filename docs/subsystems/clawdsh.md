...
This page owns the generated Cordis projection for services added by ClawDSH. Canonical messaging uses [`ctx.channels`](../../packages/openclaw/channel/README.md), while the source-checkout migration adapters remain isolated behind [`ctx.legacyChannels`](../specs/feature-channel-core.md). The two services never alias each other. `ctx.embeddings` is the independent text-vector seam described by [ADR-0003](../adr/0003-embeddings-seam.md).

The `clawdsh` profile keeps both communication planes disabled by default. Its explicit switch matrix gives the locked OpenClaw sidecar precedence over the legacy adapters, so one platform account is not logged in through both implementations. Historical direct-adapter live tests do not certify the sidecar.
...
Source: [`packages/openclaw/channel-openclaw/src/index.ts:70`](../../packages/openclaw/channel-openclaw/src/index.ts)
...
<a id="ctxlegacychannels--legacychannelregistry"></a>

### `ctx.legacyChannels` — `LegacyChannelRegistry`

Registry of channel adapters plus the inbound routing that turns their messages into agent turns and returns each reply through its adapter.
...
Types: [ChannelAdapter](../specs/feature-channel-core.md) · [PresentationConfig](../specs/feature-channel-core.md)

Source: [`packages/openclaw/channel-core/src/index.ts:227`](../../packages/openclaw/channel-core/src/index.ts)

<a id="channel-events"></a>

### `channel/*` events

<a id="channelinbound--parallel"></a>

#### `channel/inbound` — parallel

A normalized inbound message from a channel adapter, ready to route. Adapters should dispatch this with `ctx.parallel` so provider delivery does not finish before the routed turn and durability checkpoint. Legacy `ctx.emit` producers remain accepted by the contained listener.
...
Types: [ChannelMessage](../specs/feature-channel-core.md)

Source: [`packages/openclaw/channel-core/src/index.ts:86`](../../packages/openclaw/channel-core/src/index.ts)

<a id="channeloutbound--emit"></a>

#### `channel/outbound` — emit
...
Source: [`packages/openclaw/channel-core/src/index.ts:92`](../../packages/openclaw/channel-core/src/index.ts)
