...
本页承载 ClawDSH 新增服务的 Cordis 生成投影。Canonical 通信使用 [`ctx.channels`](../../packages/openclaw/channel/README.md)；仅供源码 checkout 迁移的 adapter 隔离在 [`ctx.legacyChannels`](../specs/feature-channel-core.md) 后面，两个服务永不 alias。`ctx.embeddings` 是 [ADR-0003](../adr/0003-embeddings-seam.md) 定义的独立文本向量 seam。

`clawdsh` profile 默认关闭两个通信平面。它的显式开关矩阵优先锁定的 OpenClaw sidecar，防止同一 platform account 通过两套实现同时登录。历史 direct-adapter 真实平台测试不会认证 sidecar。
