...
`@clawdsh/dsh-channel-core` 注册多个进程内 `ChannelAdapter` 实现。Adapter 发出 `channel/inbound` 并实现文本 `send`；core 保存内存中的 per-thread Session map，串行化回合，驱动 `ctx.agents`，flush `ctx.sessions`，提取 assistant reply，发送并发出 `channel/outbound`。`channel-telegram`、`channel-feishu` 与后来加入的 `channel-discord` compatibility adapter 用各自平台 SDK 实现该约定。
...
`channel-core`、`channel-telegram`、`channel-discord` 与 `channel-feishu` 作为 legacy compatibility package 保留到 ADR-0008 替换条件通过。它们的软件包测试与历史传输工作只表明旧路径曾有何行为。没有当前带凭证证据时，所有 adapter 都不是 `certified` 或 `enabled`。
...
- 删除 legacy package 要求装配好的 production sidecar、自有无密钥 snapshot path，以及对仍用于迁移的每个平台进行新认证；适用时包括 Telegram、飞书与 Discord。
