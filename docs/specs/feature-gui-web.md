...
The Host redirects `/clawdsh` to `/clawdsh/` with HTTP 308 and preserves the query string. `/clawdsh/` is the only canonical product route. `/clawdsh/settings`, `/clawdsh/settings/`, `/clawdsh/activity`, and `/clawdsh/activity/` are deprecated aliases that redirect to it with HTTP 308 and preserve the query string. The protocol-v1 bootstrap response retains the non-trailing-slash route fields for compatibility; removing them requires a separate protocol-version change. Unknown `/clawdsh/*` paths render the product's in-app 404.
...
On a clean installation, Soul reports `新会话启用`, Memory reports `已启用` with an Ark configuration reminder when needed, Skills Hub reports `来源已启用`, Channels reports `尚未连接平台`, and Automation reports `尚未设置`. The Skills status proves only that the ClawHub-compatible source participates; actual discovery is verified during a directory scan. A configured Ark key says only that first use will verify it. Channel status combines Loader state with sanitized `ctx.channels.health()` evidence: a verified handshake reports `Gateway 与 Bridge 已认证连接`, ready accounts are counted by channel without exposing account ids, and the empty-account handshake case states that OpenClaw does not expose per-account state. Automation distinguishes no rules, saved rules while disabled, enabled with no runnable rule, normal running, and failed Loader composition. Summary counts cover only these five features.
...
Optional business plugins stay in Loader composition so their schemas remain available. Their `enabled` field controls business effects: disabled Memory registers no prompt, tools, watcher, or flush, disabled Skills Hub registers no provider, and disabled Automation creates no timer, runtime, or Automation Session while retaining its management tool. Automation settings reconcile live; Soul changes apply to new Sessions. Channel Agent is required and remains network-inert by itself.
