...
- **Current assembly**: `tools/openclaw-preset-openclaw/`
- **Product role**: the local ClawDSH frontend alongside the Feishu, Telegram, Discord, and future channel frontends

## Current baseline

`pnpm dsh --profile clawdsh` composes the stock `dsh-web-app` bundle with the `clawdsh` agent preset. The browser starts at `http://127.0.0.1:3080`, new Sessions default to the preset displayed as `ClawDSH 模式`, and a GUI conversation receives Soul, Memory, Skills, and the standard agent tools. Feishu, Telegram, Discord, and Automation are disabled in a clean installation, so the Web Host starts without their credentials.
...
- Do not change the Feishu, Telegram, Discord, or other channel frontend paths.
...
| Discord | `clawdsh-discord` | disabled | restart |
...
Before the Settings control plane ships, Feishu, Telegram, Discord, and Automation use Loader `disabled` rows to establish the clean-install defaults above. The Settings control-plane increment keeps their business plugins mounted and moves runtime control to validated `enabled` fields; the product UI does not expose arbitrary Loader mutation.
...
- Browser and runtime source remains nested under `tools/openclaw-preset-openclaw/` and does not enter the root Client aggregate or shipped Client Catalog.
