...
- **状态**：implemented（阶段 3 ✅，2026-08-16 修正）
...
- **决策记录**：初始调度器映射 [2026-08-14-openclaw-cron-mapping](../../.agents/notes/implemented/architecture/2026-08-14-openclaw-cron-mapping.md)；完整组合且可发现的会话修正 [2026-08-16-automation-composed-discoverable-sessions](../../.agents/notes/implemented/bug-fix/2026-08-16-automation-composed-discoverable-sessions.md)。
...
干净安装的 `clawdsh` profile 会让 Automation plugin 保持 mounted，同时配置 `enabled=false` 且不包含规则。因此 ClawDSH Settings 仍能读取其 Config schema，而关闭的业务 effect 不会创建 timer、runtime 或 Automation 会话。Automation 不会从聊天中自行推断定时工作：用户必须显式定义何时运行，以及定时 agent 要做什么。

## 非目标

- 无渠道投递（`deliver`）与主会话摘要（`System:` 行）；回复保留在专属 Automation 会话中。
- 无运行时可编辑规则、job store、`cron.add/remove/…` 工具或 CLI；Config 即规则 store。
