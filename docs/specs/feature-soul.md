...
6. ✅ **Profile layering**: `--profile clawdsh --dump-config` resolves `dsh-base` + `dsh-web-app` + the ClawDSH profile patch.
7. ✅ **Real agent mounts preset**: `--profile clawdsh` defaults new Sessions to the `clawdsh` preset displayed as `ClawDSH 模式`; see the [assembly README](../../tools/openclaw-preset-openclaw/README.md).
