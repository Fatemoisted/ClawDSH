...
The handshake must match the configured Gateway instance, startup nonce, production or canary host lock, Node engine, AgentHarness generation, and complete capability lists. Strict schemas reject unknown fields, malformed opaque ids, non-contiguous media ordinals, invalid paths, and inconsistent action or receipt subjects. The local endpoint admits one authenticated peer; disconnecting it degrades provider health but does not silently rerun an in-flight Agent turn.
...
Production is locked to OpenClaw `v2026.7.1-2` / commit `0790d9f593ad30c940ed93b5872a8cf6d6f3cf8c`, with a verified npm tarball and extracted tree. Its public chat catalog contains 27 entries: 1 core, 2 bundled, 21 repository-official, and 3 external, conventionally summarized as **24+3**. The exact list and per-package integrities live in `tools/openclaw-channel-host/channels.production.json`.
...
Current implementation evidence establishes only `cataloged`. The shipped profile contains the sidecar composition in a default-disabled group and starts no legacy adapter by default; the separately retained legacy packages are not sidecar evidence. This change ran no live Telegram or Feishu transport smoke. The final-delivery, aggregate-account-health, stable inbound-media, Windows ACL, external-governance, and owned assembled-snapshot gates above remain open. The upstream snapshot runner does not discover `packages/openclaw/`, and an upstream `examples/` modification is outside the owned surface. Resume coverage must also prove that only known Session event names are persisted while the downstream-event seam remains unavailable.
