...
| channel-discord | `channel-discord/` | `ctx.channels` | ✅（带凭证线上 e2e 待完成） |
...
| ClawDSH 组装接线 | `tools/openclaw-preset-openclaw/` | `clawdsh` profile/patch + `clawdsh` agent preset | ✅ |
...
- ✅ preset 接线：`tools/openclaw-preset-openclaw/agent.cordis.yml` 携带 `source: ./souls/assistant.md`、`mode: append`。
...
| profile patch | `system-prompt` persona → `channel-core` → `channel-telegram`（`disabled: true`）→ `channel-discord`（`disabled: true`）→ `channel-feishu`（`disabled: true`，env 凭据引用）→ `memory` → `embeddings-ark` → `skills-hub` → `automation`（`disabled: true`）→ `agent-presets.default: clawdsh` |
| 凭证 | 关闭的渠道可以缺少凭据；飞书启用后使用 env 引用，Ark 按需解析 `ARK_API_KEY`——profile 中不提交任何值 |

- ✅ 接线完整：profile patch 覆盖全部 Host 面运行时 feature，soul 由 agent preset 覆盖。
- ✅ 层次分离正确：soul 是 agent-preset 关注点，channels/memory/skills/automation 是 profile-patch 关注点。
- ✅ 飞书、Telegram、Discord 与 Automation 均以 `disabled: true` 交付，因此干净安装的 Web Host 无需这些功能的凭据即可启动。
