...
| 位置 | 内容 |
|---|---|
| `packages/openclaw/` | 唯一可重写的包域——十个可发布包：`channel-core/`、`channel-telegram/`、`channel-discord/`、`channel-feishu/`、`soul/`、`memory/`、`embeddings/`、`embeddings-ark/`、`skills-hub/`、`automation/` |
| `docs/adr/` | 0001–0007（从项目基石到已接受、实现待完成的本地 GUI 产品决策） |
| `docs/specs/` | 路线图与 feature specs、上下文/产品链/清点地图，以及微信系排除决策 |
| `docs/cookbook/telegram-e2e.{md,zh.md,i18n.yaml}` | 自有的 Telegram 带凭证 e2e 流程与有边界的真实运行证据 |
...
| `tools/` | 应用组装（`openclaw-preset-openclaw/`）、插件骨架（`openclaw-plugin-template/`）、e2e 驱动与包括 `link-clawdsh.sh` 在内的生命周期脚本；安装脚本检测到旧 `openclaw` 资产时只警告并保留 |
...
| `tsdown.config.ts` | workspace 扫描直接纳入全部十个 `packages/openclaw/` 包目录；组装、模板与决策素材位于扫描范围外 | ADR-0001 决策 4 |
| `tsconfig.base.json` | 10 条 `@clawdsh/dsh-*` `paths`（仅新增） | ADR-0001 决策 4 |
