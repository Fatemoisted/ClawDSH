...
| Location | What it is |
|---|---|
| `packages/openclaw/` | the only rewriteable package domain — the 10 publishable packages below |
...
| `tools/` | ClawDSH assembly, plugin template, scripts, and e2e drivers (`openclaw-preset-openclaw/`, `openclaw-plugin-template/`, `ark-e2e.ts`, `link-clawdsh.sh`, `sync-upstream.sh`) |
| `.github/workflows/clawdsh-*` | ClawDSH CI (`clawdsh-publish.yml`, `clawdsh-smoke.yml`) |

### The 10 publishable packages
...
| `channel-discord/` | adapter | `ctx.channels` | Discord channel adapter (Gateway WebSocket + REST outbound) |
...
Non-package material is deliberately outside this table: application assembly lives in `tools/openclaw-preset-openclaw/`, the copy-me plugin skeleton lives in `tools/openclaw-plugin-template/`, and the WeChat-family exclusion is recorded in `docs/specs/feature-channel-wechat.md`.
...
`dsh --profile <name>` stacks layers in order: the profile's bundles → the profile's `cordis.patch.yml` → the home-level patch → `--patch` overlays. A patch targets a row by id and replaces its whole config, or inserts a new row. `tools/link-clawdsh.sh` installs `tools/openclaw-preset-openclaw/profile/cordis.patch.yml` as the `clawdsh` profile and installs its `clawdsh` preset under the dsh user root.

The clean-install profile keeps Feishu, Telegram, Discord, and Automation disabled, so the Web Host can start without their credentials. Those defaults use Loader `disabled` rows only until the Settings control-plane increment moves optional behavior behind mounted plugins' `enabled` settings. Legacy `openclaw` profile and preset directories are warning-only inputs and remain untouched; the public-distribution CLI owns the managed manifest and `clawdsh doctor` repair flow.
