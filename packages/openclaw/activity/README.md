...
`page(request, { live?, inspect? })` projects standard Session history, preferring a supplied live log and falling back to validated `sessionPersistence.inspect()` events, then merges that projection with sidecars. The projector recognizes Memory tool calls/results and flush messages, channel-origin user messages, skill tool/catalog/invocation records, and `automation/run`; it never copies message content, tool arguments/results, platform identities, provider paths, or error text. Semantic duplicates prefer the standard-history record, and conflicting duplicate ids mark the page degraded.

Pages default to 50 newest records and accept at most 100. The version-1 base64url cursor binds the Session hash, canonical category filter, order, timestamp, and id; a cursor from another query fails instead of silently changing meaning. If history or sidecars are unavailable, the page still returns the other source with explicit availability and stable warnings.
