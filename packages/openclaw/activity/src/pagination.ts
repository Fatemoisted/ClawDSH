...
export function createActivityPage(
  ...
  const collapsed = collapseLifecycleRecords(merged.records)
...
/** Reconcile owner-emitted lifecycle rows only after history and sidecars have been merged. */
function collapseLifecycleRecords(records: readonly ClawdshActivityRecord[]): MergeResult {
  ...
  for (const record of records) {
    ...
    const existingIndex = lifecycles.get(key)
    if (existingIndex === undefined) {
      lifecycles.set(key, collapsed.length)
      collapsed.push(record)
      continue
    }
...
function lifecycleKey(record: ClawdshActivityRecord): string | undefined {
  if (record.kind === 'automation.run') {
    return `automation\u0000${record.sessionId}\u0000${String(record.metadata.ruleId)}\u0000${String(record.metadata.scheduledAt)}`
  }
  if (record.kind === 'channel.delivery') {
    return [
      'channel',
      record.sessionId,
      String(record.metadata.adapter),
      String(record.metadata.conversation),
      String(record.metadata.mention),
      String(record.metadata.seq),
    ].join('\u0000')
  }
  return undefined
}
