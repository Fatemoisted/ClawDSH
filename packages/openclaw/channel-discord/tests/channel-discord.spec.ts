...
describe('the Discord channel adapter', () => {
  ...
  describe('Gateway lifecycle', () => {
    ...
    it('warns when an authenticated Gateway stops on an unrecoverable close', async () => {
      const sdk = mockClient()
      const ctx = new Context()
      const warn = vi.spyOn(ctx.logger, 'warn').mockImplementation(() => {})
      const adapter = createAdapter({}, {
      ...
      sdk.emit(Events.ShardDisconnect, { code: 4004 }, 0)
      expect(adapter.capabilities.receive).toBe(false)
      expect(warn).toHaveBeenCalledWith(expect.stringContaining('unrecoverable close 4004'))
      expect(warn.mock.calls.flat().join(' ')).not.toContain(TEST_TOKEN)
      await dispose()
    })
    ...
    it('retries a failed client destroy before admitting the credential replacement', async () => {
      vi.useFakeTimers()
      try {
