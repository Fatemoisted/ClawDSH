/**
 * Discord channel adapter over the `ctx.legacyChannels` seam, backed by discord.js.
 *
...
function startGateway(
  ...
  const lifecycleChanged = (expectedGeneration?: number): boolean =>
    disposed || (expectedGeneration !== undefined && expectedGeneration !== generation)
  ...
  const onShardDisconnect = (event: { code: number }): void => {
    ...
    if (wasAuthenticated) {
      ctx.logger.warn(
        `channel-discord: Gateway stopped permanently after unrecoverable close ${event.code}; waiting for credential or configuration update`,
      )
    }
  }
  ...
  const connect = (): void => {
    ...
    const task = (async () => {
      const token = await resolveChannelCredential(
        ctx,
        config.botToken,
        ref,
      ...
      if (lifecycleChanged(localGeneration)) return
      if (token === undefined) {
        setReceiving(false)
  ...
  const replaceClient = async (): Promise<void> => {
    ...
    try {
      ...
      const delay = Math.min(INITIAL_LOGIN_RETRY_MS * 2 ** destroyAttempt, MAX_LOGIN_RETRY_MS)
      destroyAttempt += 1
      ctx.logger.warn(
        `channel-discord: client destroy failed: ${describe(error, activeToken)}; retrying in ${delay}ms`,
      )
      // Keep the failed client current and retry cleanup through the Harness
      // timer. A replacement is never created while the prior Gateway may
      // still be live.
      cancelRetry = ctx.timeout(() => {
        cancelRetry = undefined
        queueReplacement()
