...
**Purpose**: legacy Discord adapter over `ctx.legacyChannels` — a thin `ChannelAdapter` over discord.js Gateway and REST primitives, with direct-message, guild-channel, thread, native-reply, and acknowledgement-reaction support.
...
**Seams**: legacy-only `ctx.legacyChannels` (`@clawdsh/dsh-channel-core`), Harness credentials / launch environment, and the Harness timer. It has no alias to the canonical production `ctx.channels` service.

**Specification**: historical direct-adapter implementation · **Status**: legacy compatibility, disabled by default; keyless verification is automated, while a credentialed live Discord E2E is not claimed by this change.
