...
const { installMemoryWatch, normalizeMemoryWatchRelativePath } = await import('../src/watch.ts')
...
describe('installMemoryWatch', () => {
  it('normalizes Windows separators before applying the memory allowlist', () => {
    expect(normalizeMemoryWatchRelativePath('MEMORY.md')).toBe('MEMORY.md')
    expect(normalizeMemoryWatchRelativePath('memory\\2026-08-14.md')).toBe('memory/2026-08-14.md')
    expect(normalizeMemoryWatchRelativePath('memory\\nested\\note.md')).toBeUndefined()
  })
  ...
  it('opens a watcher on the root with the memory-file options', async () => {
    ...
    expect(typeof harness.watchers[0]?.options.ignored).toBe('function')
    await dispose()
  })
  ...
  it('closes the watcher on dispose and tolerates a failing close', async () => {
    ...
    // The rejected close is contained by the disposer, not leaked as an
    // unhandled rejection.
  })
