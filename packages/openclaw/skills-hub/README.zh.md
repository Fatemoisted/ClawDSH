...
- `ctx.subprocess`（声明 inject）：`requires.bins` 与 `requires.anyBins` 通过 Harness execution-world 的 `resolveExecutable` 接缝解析，因此 gating 与受管工具观察同一个可执行文件命名空间，且不会启动子进程；
...
- **gating 尽力而为**：二进制门通过 Harness execution-world resolver 求值，并把查找失败视为不可用；OpenClaw 的 config 驱动门（`requires.config`、`os`）未移植；
