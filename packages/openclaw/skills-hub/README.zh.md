...
- **list 时 gating**：`requires.bins`（全部在 PATH）、`requires.anyBins`（至少一个）、`requires.env`（环境变量值非空）；被排除的技能不进目录，畸形 metadata 或 gating 会失败关闭，bins 在 PATH 上探测且不起子进程；
- **仅目录 + SKILL.md**：与 OpenClaw 惯例一致；frontmatter 必须从首行开始（允许 UTF-8 BOM），无 `SKILL.md` 的目录不是技能，根目录缺失即零技能，非法文件警告不引用其内容；
...
- 0.1.0：首个版本（workspace/managed/extra 三根、`metadata.clawdbot` gating、JSON 字符串 metadata 归一化；10 个契约测试，keyless）。
...
- **gating 尽力而为**：bins 经 PATH 以 `X_OK` access 语义探测（POSIX；Windows 上 `X_OK` 恒通过）；OpenClaw 的 config 驱动门（`requires.config`、`os`）未移植；
