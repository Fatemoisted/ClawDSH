...
- **Gating at list time**: `requires.bins` (all resolvable), `requires.anyBins` (at least one resolvable), `requires.env` (env var set); gated-out skills are excluded from the catalog; binary names are resolved through `ctx.subprocess.resolveExecutable` in the Harness execution world without spawning child processes;
...
- 0.1.0: first release (workspace/managed/extra roots, `metadata.clawdbot` gating, JSON-string metadata normalization; 13 contract tests, keyless).
