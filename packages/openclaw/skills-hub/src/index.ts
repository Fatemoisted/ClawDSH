...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  ctx.skills.registerProvider(control => new ClawHubProvider(resolved, ctx.logger, control.signal))
}
  ...
  async list(options: SkillLookupOptions): Promise<readonly SkillCandidate[]> {
    ...
    for (const spec of this.rootSpecs(options)) {
      ...
      for (const skillFile of await this.scanRoot(spec, signal)) {
        ...
        if (this.config.gating && !(await passesGating(parsed.metadata, signal))) continue
        candidates.push({
          name: parsed.name,
...
async function passesGating(
  ...
  if (gates.bins !== undefined) {
    if (!isStringArray(gates.bins) || !(await everyBinOnPath(gates.bins, signal))) return false
  }
  if (gates.anyBins !== undefined) {
    if (!isStringArray(gates.anyBins) || !(await anyBinOnPath(gates.anyBins, signal))) return false
  }
...
async function everyBinOnPath(names: string[], signal?: AbortSignal): Promise<boolean> {
  for (const entry of names) {
    signal?.throwIfAborted()
    if (!(await binOnPath(entry, signal))) return false
  }
  return true
}

async function anyBinOnPath(names: string[], signal?: AbortSignal): Promise<boolean> {
  for (const entry of names) {
    signal?.throwIfAborted()
    if (await binOnPath(entry, signal)) return true
  }
  return false
}

/** Probe whether an executable of this name exists on PATH (best-effort, no child processes). */
async function binOnPath(name: string, signal?: AbortSignal): Promise<boolean> {
