...
- `tools/openclaw-preset-openclaw/` — ClawDSH 组装层（agent preset + profile 模板），由 `tools/link-clawdsh.sh` 安装；
...
| [`channel-telegram/`](channel-telegram/README.md) | Telegram 渠道 | 渠道适配器 | ClawDSH `ctx.channels`；Harness credentials/LLM/attachments/timer；grammY |
...
| [`channel-feishu/`](channel-feishu/README.md) | 飞书渠道（**发起人第一优先**） | OpenClaw `extensions/feishu` | ClawDSH `ctx.channels`；Harness credentials/timer；官方 `LarkChannel` |
...
非包组装层 `tools/openclaw-preset-openclaw/` 将 `dsh-base`、`dsh-web-app` 与这些插件组合成 `clawdsh` profile 和 `clawdsh` preset，并显示为 `ClawDSH 模式`。这条原生 dsh Web GUI 基线已实现；独立 ClawDSH 产品壳、能力 Settings、语义 Activity 与 Harness 高级入口仍按 [ADR-0007](../../docs/adr/0007-clawdsh-local-gui-product.md) 待实现。

干净安装的 `clawdsh` profile 默认关闭飞书、Telegram、Discord 与 Automation，使 Web Host 无需渠道或自动化凭据即可启动。这些默认值暂时使用 Loader `disabled` 配置项；能力 Settings 增量会将其替换为业务级 `enabled` 字段。
...
当前工作树尚未实际执行 ClawDSH npm 发布。因此在明确发布前，本地开发仍使用 `tools/link-clawdsh.sh` 及其 profile symlink。
