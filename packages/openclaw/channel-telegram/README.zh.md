...
**定位**：基于 `ctx.legacyChannels` 的旧版 Telegram 适配器——grammY `Bot` 上的一层薄 `ChannelAdapter`：以可排空的长轮询接收文本/caption 与 raster image，并以带重试的原生 topic、引用回复、reaction 与 Unicode-safe 4096-unit 分片出站。
...
**接缝**：仅供旧版兼容使用的 `ctx.legacyChannels`（`@clawdsh/dsh-channel-core`），不为规范生产 `ctx.channels` 服务提供 alias。

**规格**：历史阶段 2 交付物 · [ADR-0011](../../../docs/adr/0011-deferred-channel-images-and-address-continuity.md) · **状态**：旧版兼容、默认禁用，等待 sidecar 切换验证
