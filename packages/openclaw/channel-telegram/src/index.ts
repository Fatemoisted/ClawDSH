...
/** Strip surrounding whitespace from a token while preserving its opaque value. */
...
/** Resolve one bot token through the Harness credential layers. */
...
function startLifecycle(
  ...
  const activate = async (): Promise<void> => {
    const token = suppliedBot === undefined
      ? await resolveBotToken(ctx, config, tokenRef, resolveTokenOverride)
      : normalizeToken(config.botToken) ?? normalizeToken(suppliedBot.token)
    if (disposed) return
  ...
  const disposeCredentialUpdate = ctx.on('credentials/updated', (updated) => {
    if (suppliedBot !== undefined || normalizeToken(config.botToken) !== undefined
