...
describe('ClawDSH settings page', () => {
  it('offers a visible retry when one presentation endpoint is unavailable', async () => {
    ...
    expect(await screen.findByText(/部分状态暂时不可用/)).toBeTruthy()
    fireEvent.click(screen.getByRole('button', { name: '重新读取状态' }))

    await waitFor(() => { expect(loadCapabilities).toHaveBeenCalledTimes(2) })
    await waitFor(() => { expect(screen.queryByRole('button', { name: '重新读取状态' })).toBeNull() })
    expect(screen.queryByText(/部分状态暂时不可用/)).toBeNull()
  })
