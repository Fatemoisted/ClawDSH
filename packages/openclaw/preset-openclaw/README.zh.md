...
3. **profile 模板**（`profile/`）——复制到 `$DSH_HOME/profiles/openclaw/` 即成为 `--profile openclaw` 的组装基座（bundles：`dsh-base` + `dsh-web-app`，常驻 daemon + 浏览器 GUI；不含 `dsh-headless`，那是一次性任务跑器）。

**规格**：docs/specs/roadmap.md（阶段 0/2/4 交付物） · **状态**：phase-2 e2e-verified（飞书消息 → 人格化 agent → 回复，真实闭环已验证）+ 阶段 4 GUI 接线（web bundle + openclaw preset）
...
- ✅（阶段 4）GUI 接线——profile bundles `dsh-web-app`、默认 preset `openclaw`；`agent.cordis.yml` 镜像 `standard` 预设全套工具、用 `soul` 替换 `persona`；`link-openclaw.sh` 把 preset 装进 `$DSH_HOME/.agent-presets/openclaw/`。
...
新会话默认挂「OpenClaw 形态」preset（soul 人格 + memory 召回 + skills + 全套工具）；`profile/cordis.patch.yml` 把 web bundle 的 `agent-presets` 默认改成 `openclaw`。
