 ...
 */
export async function ordinaryFileTreeDigest(root: string): Promise<OrdinaryFileTreeLock> {
  const canonicalRoot = await realpath(root)
  const files = (await ordinaryFiles(canonicalRoot)).sort()
  const tree = createHash('sha512')
  for (const path of files) {
    const info = await lstat(path)
    const relativePath = relative(canonicalRoot, path).split(sep).join('/')
    const contentDigest = await sha512File(path)
...
export async function installedProjectTreeDigest(
  ...
  const entries = await installedEntries(canonicalRoot, canonicalRoot, excluded)
  ...
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    ...
    if (entry.isSymbolicLink()) {
      ...
      entries.push({
        kind: 'link',
        logicalPath,
        targetPath: relative(root, await realpath(target)).split(sep).join('/'),
        size: targetInfo.size,
        sha512: await sha512File(target),
      })
    } else if (entry.isDirectory()) {
      entries.push(...await installedEntries(root, path, excludedExternalSymlinks))
    } else if (entry.isFile()) {
      const info = await lstat(path)
      entries.push({ kind: 'file', logicalPath, size: info.size, sha512: await sha512File(path) })
    } else {
...
async function ordinaryFiles(root: string, directory = root): Promise<string[]> {
  const files: string[] = []
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) files.push(...await ordinaryFiles(root, path))
