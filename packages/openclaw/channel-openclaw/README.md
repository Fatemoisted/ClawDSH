...
The OpenClaw JSON must replace the model registry with only `clawdsh/local`, select the `clawdsh` AgentHarness for every default and named Agent route, use an empty fallback list, and place the Agent workspace at `stateDir/workspace`. `plugins.load.paths` contains only the verified bridge root; `plugins.allow` and enabled `plugins.entries` contain exactly `clawdsh-bridge` plus locked extension ids; `plugins.installs` is empty. Bash, config, MCP, plugin, debug, restart, and native-skill commands are explicitly disabled; admitted text commands require access groups, wildcard senders are forbidden, global and every named Agent must set `tools.elevated.enabled: false`, and Agent defaults must set `elevatedDefault: off`. A `channels` object is required even when it is empty, and every entry and account must set `enabled` explicitly. The first locked admission validators allow enabled Telegram and Feishu configurations: both require `configWrites: false`, safe DM and group policies, and explicit mention admission through Feishu's `requireMention` or Telegram's `groups["*"].requireMention`. Other cataloged Channels must remain disabled until their version-specific admission fields receive a validator; this prevents the catalog from being mistaken for certified runtime support. Unsafe nested policy values and public wildcard senders are rejected for every Channel. A mismatch fails before the Gateway starts.
...
This test validates credential-free, policy-complete Telegram and Feishu configs with the real stable OpenClaw schema, starts the verified production Gateway, traverses the stable V1 bridge and the real DSH `channel` and `channel-agent` packages, and obtains a terminal answer from a deterministic mock LLM. It then disconnects the Provider and proves a second Gateway request returns the bridge failure with `fallbackUsed: false` and without another DSH model call.
...
- **Certification is external evidence** — keyless tests verify this Provider and bridge protocol, not live account credentials, platform terms, hardware dependencies, or per-Channel certification.
