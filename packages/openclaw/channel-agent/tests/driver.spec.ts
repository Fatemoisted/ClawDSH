...
describe('channel Agent turn execution', () => {
  ...
  it('disposes a candidate whose startup quiescence fails and retries the deterministic Session', async () => {
    const app = await harness([textResponse('recovered')])
    const originalCreate = app.ctx.agents.create.bind(app.ctx.agents)
    let rejectFirstCandidate = true
    let disposalCalls = 0
    const create = vi.spyOn(app.ctx.agents, 'create').mockImplementation(async (options) => {
      const handle = await originalCreate(options)
      if (!rejectFirstCandidate) return handle
      rejectFirstCandidate = false
      vi.spyOn(handle.agent, 'whenIdle').mockRejectedValueOnce(new Error('late startup failure'))
    ...
    expect(app.ctx.agents.get(sessionIdFor(turn().route))).toBeUndefined()
    expect(app.ctx.agents.list()).toEqual([])
  ...
  it('preserves startup and cleanup failures when a failed candidate cannot dispose cleanly', async () => {
    const app = await harness([], { mountAgent: false })
    const startupFailure = new Error('late startup failure')
    const cleanupFailure = new Error('candidate cleanup failure')
    const originalCreate = app.ctx.agents.create.bind(app.ctx.agents)
    vi.spyOn(app.ctx.agents, 'create').mockImplementationOnce(async (options) => {
      const handle = await originalCreate(options)
      vi.spyOn(handle.agent, 'whenIdle').mockRejectedValueOnce(startupFailure)
      return {
        agent: handle.agent,
        async dispose(): Promise<void> {
    ...
    const acquire = (driver as unknown as {
      acquire: (route: ChannelTurnEnvelopeV1['route'], sessionId: ReturnType<typeof sessionIdFor>, preset: string) => Promise<AgentHandle>
    }).acquire.bind(driver)

    let failure: unknown
    try {
      await acquire(turn().route, sessionIdFor(turn().route), 'owner')
    } catch (error) {
      failure = error
    }
