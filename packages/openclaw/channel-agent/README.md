...
The first accepted generation becomes current for its Gateway/session lineage. `session.reset` retires the exact current generation and admits only a strictly larger generation; it cancels and drains matching live work but keeps the idle Agent handle so the historical Session remains visible in the host. `session.close` retires the exact named generation, cancels matching live work, and disposes its owned Agent handle. Their generation commit atomically records the exact control request and reset acknowledgement, so a bridge retry after a lost acknowledgement replays the completed control without invalidating the successor. A stale or closed generation cannot enter the model, including a retry of a previously accepted turn.
...
When the host installs its optional title and workspace presentation services, every newly created or resumed channel Session becomes visible in the ordinary Web workspace after its Agent reaches idle. An untitled Session receives `外部消息 · <channel> · 私聊` or `外部消息 · <channel> · 群聊`; the title uses only the channel name and conversation kind, never message text, account or conversation ids, or sender names. An existing title is never overwritten. A new Session records the running `cwd`; a resumed Session keeps the `cwd` in its immutable header even when restart configuration differs. Workspace publication resolves that recorded path and attaches the Session when a matching workspace exists. A legacy header without `cwd` still receives title handling but skips workspace attachment with a fixed, sanitized warning. These best-effort presentation writes never change the channel turn result.
