...
**接缝**：`ctx.channels`（@clawdsh/dsh-channel-core，ADR-0002）、Harness credentials / 启动环境与 Harness timer。与 Telegram（轮询）形态互补，共同验证渠道 seam。

**规格**：docs/specs/roadmap.md（阶段 2 交付物） · **状态**：implemented；下述永不结束握手的 timeout 待完成
