...
**规格**：docs/specs/roadmap.md（阶段 2 交付物） · **状态**：implemented；Harness credentials 与下述永不结束握手的 timeout 待完成
...
- **带凭证 e2e**：无密钥测试已覆盖归一化映射、可等待入站、WebSocket 前退避、握手拒绝后的清理、Unicode-safe topic 分片、原生引用与 reaction；线上 app 权限、事件订阅及 WebSocket 行为仍需飞书/Lark 部署验证。
