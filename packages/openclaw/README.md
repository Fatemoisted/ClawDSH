...
|---|---|---|---|---|
| `preset-openclaw/` | internal source for the `clawdsh` profile, private development bundle, product shell, brand, and release assembly | overall assembly | isolated source profile and public dsh Web assembly | **implemented** |
| `preset-openclaw/distribution/bundle/` | public ClawDSH composition bundle | managed product assets | dsh bundle patch and exact package dependencies | **public RC payload: `0.1.0-rc.1@next`** |
| `preset-openclaw/distribution/cli/` | managed installer, launcher, source migration, doctor, and explicit Channel installer | local product installation | exact dsh CLI dependency and managed filesystem assets | **public RC payload: `0.1.0-rc.1@next`** |
...
The functional release tooling builds real tarballs, converts owned `workspace:` relations to exact `0.1.0-rc.1` dependencies, rejects local protocols, symlinks, undeclared files, and private registry URLs, and exercises the packages through a temporary registry and isolated dsh home.

Package-name creation uses a separate, reviewed `0.1.0-rc.0@bootstrap` set. Each deterministic inert archive contains only package metadata, the MIT license, and a README that directs users to `@next`; it has no dependency, executable, export, entry point, script, or code file. A closed `bootstrap-index.json` records SHA-512 for all thirteen archives. Generate that set, publish each package, and run the `0.1.0-rc.1` workflow from the same reviewed commit: changing the root `LICENSE` or bootstrap contract changes the immutable archive SHA-512, so the inspector and workflow fail closed. The read-only publication helper accepts an existing remote version only when its integrity matches, rejects every `latest` tag, and prints one explicit interactive-2FA `npm publish ... --tag bootstrap` command at a time. The [bootstrap operator guide](preset-openclaw/distribution/release-tools/bootstrap/README.md) owns the exact commands and recovery procedure.

After package creation, every trusted-publisher record must match `clawdsh-publish.yml`, GitHub environment `npm`, and `npm publish`; that environment admits only branch `clawdsh`, and release readiness requires `refs/heads/clawdsh`. The artifact-first workflow publishes the functional `0.1.0-rc.1` archives with OIDC provenance under `next` and refuses to create `latest`. [ADR-0009](../../docs/adr/0009-public-npm-distribution.md) owns the publication conditions.
