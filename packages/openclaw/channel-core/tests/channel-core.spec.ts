...
async function persistentHarness(root: string, adapter: MockAdapter): Promise<Context> {
  ...
  ctx.llm.registerAdapter(['mock'], adapter)
  await ctx.plugin(LegacyChannelRegistry, { agentPreset: 'openclaw', ackReactionScope: 'all' })
  return ctx
}
...
describe('the channel-core seam', () => {
  ...
  it('binds a helper-registered adapter to the owning plugin lifecycle', async () => {
    ...
    const owner = await ctx.plugin(Object.assign((inner: Context) => {
      registerLegacyChannelAdapter(inner, () => ({
        id: 'owned',
        capabilities: { receive: true, send: true, react: false },
        start: () => () => { stop() },
        send: async () => {},
      }))
    }, { inject: ['legacyChannels'] }))

    expect(ctx.legacyChannels.getAdapter('owned')).toBeDefined()
    await owner.dispose()
    expect(ctx.legacyChannels.getAdapter('owned')).toBeUndefined()
  ...
  it('rolls registration back when an adapter throws during startup', async () => {
    ...
    expect(() => ctx.legacyChannels.registerAdapter(broken)).toThrow(/startup failed/)
    expect(ctx.legacyChannels.getAdapter('fake')).toBeUndefined()

    const dispose = ctx.legacyChannels.registerAdapter(fakeAdapter([]))
    expect(ctx.legacyChannels.getAdapter('fake')).toBeDefined()
    await dispose()
  })
  ...
  it('keeps a caption turn on text-only models without downloading its image', async () => {
    ...
    const sent: ChannelMessage[] = []
    const materializeImages = vi.fn(async () => [])
    ctx.legacyChannels.registerAdapter({ ...fakeAdapter(sent), materializeImages })
  ...
  it('does not materialize an unmentioned group image', async () => {
    ...
    const ctx = await harness(llm)
    const materializeImages = vi.fn(async () => [])
    ctx.legacyChannels.registerAdapter({ ...fakeAdapter([]), materializeImages })
  ...
  it('keeps the adapter registered while its stop drains queued replies', async () => {
    ...
    const dispose = ctx.legacyChannels.registerAdapter({
      id: 'fake',
      capabilities: { receive: true, send: true, react: false },
  ...
  it('resumes the same deterministic JSONL session after a process-style restart', async () => {
    ...
    try {
      ...
      first.legacyChannels.registerAdapter(fakeAdapter([]))
      ...
      second.legacyChannels.registerAdapter(fakeAdapter([]))
