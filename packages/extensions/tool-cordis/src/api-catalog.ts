...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  {
    key: 'legacyChannels',
    summary: 'Registry of channel adapters plus the inbound routing that turns their messages into agent turns and returns each reply through its adapter.',
    description: 'Registry of channel adapters plus the inbound routing that turns their messages into agent turns and returns each reply through its adapter.',
    methods: [
      {
        signature: 'getPresentation(): Readonly<PresentationConfig>',
        description: 'The registry\'s presentation config — the single entry point adapters use to derive mention patterns from the deployment identity.',
        parameters: [],
        returns: 'the resolved presentation config, readonly.',
      },
      {
        signature: 'registerAdapter(adapter: ChannelAdapter): () => Promise<void>',
        description: 'Register an adapter and start it, failing loud on a duplicate id.',
        parameters: [{ name: 'adapter', description: 'the adapter to register; its `id` must be unique among live adapters.' }],
        returns: 'the disposer that stops the adapter and removes it from the registry.',
      },
      {
        signature: 'getAdapter(id: string): ChannelAdapter | undefined',
        description: 'Look up a registered adapter by id.',
        parameters: [{ name: 'id', description: 'the adapter id.' }],
        returns: 'the adapter, or `undefined` when no such adapter is registered.',
      },
      {
        signature: 'listAdapters(): ChannelAdapter[]',
        description: 'List every live adapter.',
        parameters: [],
...
export const EVENT_API: readonly EventApiEntry[] = [
  ...
  },
  {
    ...
    mode: 'parallel',
    signature: '\'channel/inbound\'(message: ChannelMessage): Promise<void> | void',
    summary: 'A normalized inbound message from a channel adapter, ready to route.',
    description: 'A normalized inbound message from a channel adapter, ready to route. Adapters should dispatch this with `ctx.parallel` so provider delivery does not finish before the routed turn and durability checkpoint. Legacy `ctx.emit` producers remain accepted by the contained listener.',
    parameters: [{ name: 'message', description: 'the inbound message routed to its per-thread agent.' }],
  },
  {
    name: 'channel/outbound',
    mode: 'emit',
    signature: '\'channel/outbound\'(message: ChannelMessage): void',
    summary: 'A reply delivered back through its owning adapter.',
    description: 'A reply delivered back through its owning adapter.',
    parameters: [{ name: 'message', description: 'the outbound reply that was delivered.' }],
  },
  {
...
export const TYPE_API: readonly TypeApiEntry[] = [
  ...
  },
  {
    ...
    declaration: 'export interface ChannelCapabilities {\n    receive: boolean;\n    send: boolean;\n    react: boolean;\n}',
  },
  {
  ...
  {
    name: 'ChannelImageSource',
    declaration: 'export interface ChannelImageSource {\n    sourceId: string;\n    mediaType: ImageMediaType;\n    bytes?: number;\n    name?: string;\n}',
  },
  {
    name: 'ChannelMediaId',
  ...
  {
    name: 'IdentityConfig',
    declaration: 'export interface IdentityConfig {\n    name?: string;\n    theme?: string;\n    emoji?: string;\n}',
  },
  {
  ...
  },
  {
    ...
    declaration: 'export interface PresentationConfig {\n    identity?: IdentityConfig;\n    responsePrefix?: string;\n    ackReaction?: string;\n}',
  },
  {
    name: 'PresetOption',
